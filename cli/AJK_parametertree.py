# -*- coding: utf-8 -*-
"""
Display the detector parameters in a GUI
"""
...
# This parameter automatically generates
# two child parameters which are always reciprocals of each other
class ComplexParameter(pTypes.GroupParameter):
    def __init__(self, **opts):
        ...
        self.addChild({'name': 'A = 1/B',
                       'type': 'float',
                       'value': 7,
                       'suffix': 'Hz',
                       'siPrefix': True})
        self.addChild({'name': 'B = 1/A',
                       'type': 'float',
                       'value': 1/7.,
                       'suffix': 's',
                       'siPrefix': True})
...
# this group includes a menu allowing the user to add new parameters
# into its child list
...
def save():
    ...
    mars_msw = (tm | sbm | saux | sp | slh | gn | c0 | ss | tr | sse |
                spur | rt | t0 | sl | sb | sbn | m1 | m0 | senf2 | senf1 | rm |
                pb)
    ...
    mars_mid13 = (
        (int(p2.param('Global parameters', 'PB0:PB9').value()) & 127) << 25 |
        (int(p2.param('Global parameters', 'PA0:PA9').value())) << 15 |
        mars_int)
